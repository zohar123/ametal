...
//    /** \brief 获取SDIO控制器信息 */
    ...
    int (*pfn_sdcard_get_sdinfo) (void *p_drv,  am_sd_cardinfo_t *p_sdinfo);
 ...
 * \brief SDIO 标准服务结构体
 */
typedef struct am_sdcard_serv {
	const struct am_sdcard_drv_funcs *p_funcs;    /**< \brief SDIO  sdio驱动函数     */
    void                             *p_drv;      /**< \brief SDIO  sdio驱动函数参数 */
} am_sdcard_serv_t;

/** \brief SDIO 标准服务操作句柄定义 */
typedef am_sdcard_serv_t *am_sdcard_handle_t;

/**
 * \brief SDCARD 设备信息结构体
 */
typedef struct am_sd_card_devinfo{
...
typedef struct am_sdcard_dev{
	...
	struct am_sdcard_serv   sdcard_serv;
	am_sd_card_devinfo_t   *dev_info;
	am_sd_cardinfo_t        sdcard_info;
	am_wait_t               wait;
}am_sdcard_dev_t;
...
{
	return handle->p_funcs->pfn_sdcard_rest(handle->p_drv);
}

//    /** \brief 获取SDIO控制器信息 */
am_static_inline
int am_sdcard_read (am_sdcard_handle_t handle, uint8_t *p_buf, uint32_t addr, uint16_t block_size) {
	return handle->p_funcs->pfn_sdcard_read(handle->p_drv, p_buf, addr, block_size);
}

am_static_inline
int am_sdcard_write (am_sdcard_handle_t handle, uint8_t *p_buf, uint32_t addr, uint16_t block_size) {
	return handle->p_funcs->pfn_sdcard_write(handle->p_drv, p_buf, addr, block_size);
}

am_static_inline
int am_sdcard_erase (am_sdcard_handle_t handle, uint32_t addr_start, uint32_t addr_end) {
	return handle->p_funcs->pfn_sdcard_erase(handle->p_drv, addr_start, addr_end);
}

am_static_inline
int am_sdcard_get_sdinfo (am_sdcard_handle_t handle, am_sd_cardinfo_t *p_sdinfo) {
	return handle->p_funcs->pfn_sdcard_get_sdinfo(handle->p_drv, p_sdinfo);
}

am_static_inline
int am_sdcard_stop (am_sdcard_handle_t handle) {
	return handle->p_funcs->pfn_sdcard_stop(handle->p_drv);
}
