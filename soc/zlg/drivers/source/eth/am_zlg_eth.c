...
IS_AM_ZLG_ETH_AUTO_NEGOTIATION(p_eth_regs_config->eth_auto_negotiation));
    __assert_param(IS_AM_ZLG_ETH_WATCH_DOG(p_eth_regs_config->eth_watch_dog));
    __assert_param(IS_AM_ZLG_ETH_JABBER(p_eth_regs_config->eth_jabber));
    __assert_param(
IS_AM_ZLG_ETH_INTER_FRAME_GAP(p_eth_regs_config->eth_inter_fame_gap));
    __assert_param(IS_AM_ZLG_ETH_SPEED(p_eth_regs_config->eth_speed));
    __assert_param(
...
IS_AM_ZLG_ETH_LOOPBACK_MODE(p_eth_regs_config->eth_loop_back_mode));
    __assert_param(IS_AM_ZLG_ETH_DUPLEX_MODE(p_eth_regs_config->eth_mode));
    __assert_param(
...
IS_AM_ZLG_ETH_UNICAST_FRAMES_FILTER(p_eth_regs_config->eth_unicast_frames_filter));
    __assert_param(IS_AM_ZLG_ETH_PAUSE_TIME(p_eth_regs_config->eth_pause_time));
    __assert_param(
...
{
    amhw_zlg_eth_mac_transmission_cmd(ENABLE);
    amhw_zlg_eth_mac_reception_cmd(ENABLE);
    amhw_zlg_eth_flush_transmit_fifo();
    amhw_zlg_eth_dma_transmission_cmd(ENABLE);
    amhw_zlg_eth_dma_reception_cmd(ENABLE);
}
...
{
    amhw_zlg_eth_dma_transmission_cmd(DISABLE);
    amhw_zlg_eth_dma_reception_cmd(DISABLE);
    amhw_zlg_eth_mac_reception_cmd(DISABLE);
    amhw_zlg_eth_flush_transmit_fifo();
    amhw_zlg_eth_mac_transmission_cmd(DISABLE);
}
...
uint32_t am_zlg_eth_prepare_transmit_desc(uint16_t frame_length)
{
    ...
    if ((ETH->DMASR & AM_ZLG_ETH_DMASR_TBUS) != (uint32_t) RESET) {
        ETH->DMASR = AM_ZLG_ETH_DMASR_TBUS;
        ETH->DMATPDR = 0;
    }
 ...
 * \retval None
 */
void am_zlg_eth_dma_rx_desc_init(eth_dma_desc_type_t *p_dma_rx_desc_tab,
 ...
 * \retval None
 */
void am_zlg_eth_dma_tx_desc_init(eth_dma_desc_type_t *p_dma_tx_desc_tab,
 ...
 * \retval None
 */
static void am_eth_mac_dma_config(void)
{
    AM_DBG_INFO("\r\n __am_eth_mac_dma_config! \r\n");

    amhw_zlg_eth_soft_reset();
    while (amhw_zlg_eth_get_soft_reset_status() == SET)
        ;
    amhw_zlg_eth_usr_config_init(&g_eth_regs_config);
    am_eth_check_usr_config(&g_eth_regs_config);
    g_eth_status = amhw_zlg_eth_reg_init(&g_eth_regs_config,
...
{
    amhw_zlg_eth_gpio_config();
    am_eth_mac_dma_config();
    if (AMHW_ZLG_ETH_GET_PHY_LINK_STATUS()) {
...
void am_zlg_eth_check_link_status(uint16_t phy_addr)
{
    ...
    uint32_t tmp = AMHW_ZLG_ETH_GET_PHY_LINK_STATUS();
...
void am_zlg_eth_link_callback(struct netif *netif)
{
    ...
    if (netif_is_link_up(netif)) {
        if (g_eth_regs_config.eth_auto_negotiation
            ...
            timeout = 0;
            am_zlg_phy_write_reg(ETHERNET_PHY_ADDRESS, AM_ZLG_PHY_BCR,
            AM_ZLG_PHY_AUTO_NEGOTIATION);
            do {
                timeout++;
            } while (!(am_zlg_phy_read_reg(ETHERNET_PHY_ADDRESS,
                    AM_ZLG_PHY_BSR) & AM_ZLG_PHY_AUTO_NEGO_COMPLETE)
                    && (timeout < (uint32_t) AM_ZLG_PHY_READ_TO));
            timeout = 0;
            reg_value = am_zlg_phy_read_reg(ETHERNET_PHY_ADDRESS,
                    AM_ZLG_PHY_SR);
            ...
            amhw_zlg_eth_set_speed(eth_speed);
            amhw_zlg_eth_set_mode(eth_mode);
        }
        am_zlg_eth_start();
