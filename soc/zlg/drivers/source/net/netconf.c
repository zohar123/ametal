...
void am_eth_lwip_dhcp_process_handle(void);
#endif
...
void am_eth_lwip_init(void)
{
    ...
    if (g_eth_status == (AM_ETH_INIT_FLAG | AM_ETH_LINK_FLAG)) {
...
#ifdef SERIAL_DEBUG
        printf("\n  Static IP address   \n");
        printf("IP: %d.%d.%d.%d\n",AM_ETH_IP_ADDR0,AM_ETH_IP_ADDR1,AM_ETH_IP_ADDR2,AM_ETH_IP_ADDR3);
        printf("NETMASK: %d.%d.%d.%d\n",AM_ETH_NETMASK_ADDR0,AM_ETH_NETMASK_ADDR1,AM_ETH_NETMASK_ADDR2,AM_ETH_NETMASK_ADDR3);
        printf("Gateway: %d.%d.%d.%d\n",AM_ETH_GW_ADDR0,AM_ETH_GW_ADDR1,AM_ETH_GW_ADDR2,AM_ETH_GW_ADDR3);
#endif /* SERIAL_DEBUG */
#endif /* USE_DHCP */
    } else {
    ...
    netif_set_link_callback(&g_eth_netif, am_eth_link_callback);
}
...
{
    am_eth_ethernetif_input(&g_eth_netif);
}
...
void am_eth_lwip_periodic_handle(__IO uint32_t localtime)
{
    if ((localtime + TCP_TMR_INTERVAL >= 4294967295UL)
            ...
            || (localtime + AM_ETH_LINK_TIMER_INTERVAL >= 4294967295UL)
#ifdef USE_DHCP
        ...
        tcp_tmr();
        etharp_tmr();
        am_eth_check_link_status(ETHERNET_PHY_ADDRESS);
#ifdef USE_DHCP
        dhcp_fine_tmr();
        if ((am_eth_dhcp_state != AM_ETH_DHCP_ADDRESS_ASSIGNED) &&
                (am_eth_dhcp_state != AM_ETH_DHCP_TIMEOUT) &&
                (am_eth_dhcp_state != AM_ETH_DHCP_LINK_DOWN))
        {
            am_eth_lwip_dhcp_process_handle();
        }
        dhcp_coarse_tmr();
#endif
    }
    ...
    if ((localtime - LinkTimer) >= AM_ETH_LINK_TIMER_INTERVAL) {
        LinkTimer = localtime;
        am_eth_check_link_status(ETHERNET_PHY_ADDRESS);
    }
...
#ifdef USE_DHCP
/**
 * @brief  am_eth_lwip_dhcp_process_handle
 * @param  None
 * @retval None
 */
void am_eth_lwip_dhcp_process_handle(void)
{
    ...
    {
        case AM_ETH_DHCP_START:
        {
            am_eth_dhcp_state = AM_ETH_DHCP_WAIT_ADDRESS;
        ...
        case AM_ETH_DHCP_WAIT_ADDRESS:
        {
            ...
            if (IPaddress!=0)
            {
                am_eth_dhcp_state = AM_ETH_DHCP_ADDRESS_ASSIGNED;
            ...
            else
            {
                ...
                if (g_eth_netif.dhcp->tries > MAX_DHCP_TRIES)
                {
                    ...
                    /* Static address used */
                    IP4_ADDR(&ipaddr, AM_ETH_IP_ADDR0 ,AM_ETH_IP_ADDR1 , AM_ETH_IP_ADDR2 , AM_ETH_IP_ADDR3 );
                    IP4_ADDR(&netmask, AM_ETH_NETMASK_ADDR0, AM_ETH_NETMASK_ADDR1, AM_ETH_NETMASK_ADDR2, AM_ETH_NETMASK_ADDR3);
                    IP4_ADDR(&gw, AM_ETH_GW_ADDR0, AM_ETH_GW_ADDR1, AM_ETH_GW_ADDR2, AM_ETH_GW_ADDR3);
