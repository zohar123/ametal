 ...
 */
void demo_am824_hw_acmp_int_entry (void);

/**
 * \brief 模拟比较器例程（电压阶梯），通过 HW 层接口实现
 */
void demo_am824_hw_acmp_lad_entry (void);

/**
 * \brief 模拟比较器例程（查询模式），通过 HW 层接口实现
 */
void demo_am824_hw_acmp_poll_entry (void);

/**
 * \brief ADC 例程，转换结果数据通过 DMA 传输，通过 HW 层接口实现
 */
void demo_am824_hw_adc_dma_entry (void);

/**
 * \brief ADC 阈值比较例程，通过 HW 层接口实现
 */
void demo_am824_hw_adc_thcmp_entry (void);

/**
 * \brief ADC DMA 例程（中断方式），通过标准接口实现
 */
void demo_am824_std_adc_dma_int_entry (void);

/**
 * \brief ADC DMA 例程（轮询方式），通过标准接口实现
 */
void demo_am824_std_adc_dma_poll_entry (void);

/**
 * \brief ADC 例程（中断方式），通过标准接口实现
 */
void demo_am824_std_adc_int_entry (void);

/**
 * \brief ADC 利用板载热敏电阻 (NTC) 进行温度测量例程，通过标准接口实现
 */
void demo_am824_std_adc_ntc_entry (void);

/**
 * \brief ADC 例程（轮询方式），通过标准接口实现
 */
void demo_am824_std_adc_poll_entry (void);

/**
 * \brief 蜂鸣器例程，通过标准接口实现
 */
void demo_am824_std_buzzer_entry (void);
 ...
 */
void demo_am824_std_crc_entry (void);
 ...
 */
void demo_am824_drv_dma_m2m_entry (void);

/**
 * \brief DMA 乒乓传输例程，通过驱动层接口实现
 */
void demo_am824_drv_dma_ping_pong_entry (void);

/**
 * \brief 闪存控制器例程，通过 HW 层接口实现
 */
void demo_am824_hw_flash_ctrl_entry (void);
 ...
 * \brief GPIO 引脚中断例程，通过标准接口实现
 */
void demo_am824_std_gpio_int_entry (void);
 ...
 */
void demo_am824_std_gpio_entry (void);

/**
 * \brief I2C 读写 EEPROM 例程（DMA 方式），通过 HW 层接口实现
 */
void demo_am824_hw_i2c_master_dma_entry (void);

/**
 * \brief I2C 读写 EEPROM 例程（轮询方式），通过 HW 层接口实现
 */
void demo_am824_hw_i2c_master_poll_entry (void);
 ...
 */
void demo_am824_hw_i2c_slave_poll_entry (void);

/**
 * \brief I2C 主机从 LM75 传感器读取温度值例程，通过标准接口实现
 */
void demo_am824_std_i2c_lm75_entry (void);

/**
 * \brief I2C 主机读写 EEPROM 例程，通过异步标准接口实现
 */
void demo_am824_std_i2c_master_async_entry (void);
 ...
 */
void demo_am824_std_key_input_event_entry (void);
 ...
 */
void demo_am824_hw_mrt_entry (void);

/**
 * \brief 多频率定时器（MRT）例程，通过标准接口实现
 */
void demo_am824_std_mrt_timer_entry (void);

/**
 * \brief PLL 例程，通过 HW 层接口实现
 */
void demo_am824_hw_pll_entry (void);

/**
 * \brief PMU 深度掉电模式例程，通过 HW 层接口实现
 */
void demo_am824_hw_pmu_deeppowerdown_entry (void);

/**
 * \brief PMU 深度睡眠模式例程，通过 HW 层接口实现
 */
void demo_am824_hw_pmu_deepsleep_entry (void);

/**
 * \brief PMU 掉电模式例程，通过 HW 层接口实现
 */
void demo_am824_hw_pmu_powerdown_entry (void);

/**
 * \brief PMU 睡眠模式例程，通过 HW 层接口实现
 */
void demo_am824_hw_pmu_sleep_entry (void);

/**
 * \brief SCT 32 位 PWM 例程，通过驱动层接口实现
 */
void demo_am824_drv_sct_timing_pwm_entry (void);
 ...
 */
void demo_am824_hw_sct_1_32bit_timing_entry (void);

/**
 * \brief SCT 作为 2 个 16 位定时器输出 PWM 例程，通过 HW 层接口实现
 */
void demo_am824_hw_sct_2_16bit_pwm_entry (void);

/**
 * \brief SCT 作为 2 个 16 位定时器定时例程，通过 HW 层接口实现
 */
void demo_am824_hw_sct_2_16bit_timing_entry (void);
 ...
 */
void demo_am824_hw_sct_multi_states_entry (void);

/**
 * \brief SCT 捕获例程，通过标准接口实现
 */
void demo_am824_std_sct_cap_entry (void);

/**
 * \brief SCT PWM 例程，通过标准接口实现
 */
void demo_am824_std_sct_pwm_entry (void);

/**
 * \brief SCT 定时例程，通过标准接口实现
 */
void demo_am824_std_sct_timer_entry (void);

/**
 * \brief SPI 主机发送数据例程，通过 HW 层接口实现
 */
void demo_am824_hw_spi_master_entry (void);
 ...
 */
void demo_am824_std_spi_flash_dma_entry (void);
 ...
 */
void demo_am824_std_spi_master_dma_entry (void);

/**
 * \brief SPI 回环例程（中断方式），通过标准接口实现
 */
void demo_am824_std_spi_master_int_entry (void);
 ...
 */
void demo_am824_hw_clkout_entry (void);

/**
 * \brief Systick 延时例程，通过标准接口实现
 */
void demo_am824_std_delay_entry (void);

/**
 * \brief Systick 例程，通过标准接口实现
 */
void demo_am824_std_systick_timer_entry (void);
 ...
 */
void demo_am824_hw_usart_rx_dma_entry (void);
 ...
 */
void demo_am824_std_usart_flowctrl_entry (void);

/**
 * \brief USART 例程（轮询方式），通过标准接口实现
 */
void demo_am824_std_usart_poll_entry (void);

/**
 * \brief USART 环形缓冲区例程，通过标准接口实现
 */
void demo_am824_std_usart_rngbuf_entry (void);

/**
 * \brief WKT 深度掉电唤醒例程，通过 HW 层接口实现
 */
void demo_am824_hw_wkt_deeppowerdown_wakeup_entry (void);

/**
 * \brief WKT 定时器例程，通过 HW 层接口实现
 */
void demo_am824_hw_wkt_timer_entry (void);

/**
 * \brief WKT 定时器例程，通过标准接口实现
 */
void demo_am824_std_wkt_timer_entry (void);
 ...
 */
void demo_am824_microport_ds1302_entry (void);

/**
 * \brief MicroPort-EEPROM 例程，通过 NVRAM 标准接口实现
 */
void demo_am824_microport_eeprom_nvram_entry (void);
 ...
 */
void demo_am824_microport_flash_ftl_entry (void);

/**
 * \brief MicroPort-FLASH 例程，通过 MTD 标准接口实现
 */
void demo_am824_microport_flash_mtd_entry (void);

/**
 * \brief MicroPort-FLASH 例程，通过标准接口实现
 */
void demo_am824_microport_flash_entry (void);

/**
 * \brief MicroPort-RS485 例程，通过标准接口实现
 */
void demo_am824_microport_rs485_entry (void);

/**
 * \brief MicroPort-RTC(PCF85063) 例程，通过标准接口实现
 */
void demo_am824_microport_rtc_entry (void);
 ...
 */
void demo_am824_microport_usb_entry (void);

/**
 * \brief MiniPort-View 例程，通过标准接口实现
 */
void demo_am824_miniport_digitron_entry (void);

/**
 * \brief MiniPort-595 + MiniPort-View + MiniPort-KEY 例程，通过标准接口实现
 */
void demo_am824_miniport_hc595_digitron_key_entry (void);
 ...
 */
void demo_am824_miniport_hc595_digitron_entry (void);
 ...
 */
void demo_am824_miniport_key_entry (void);

/**
 * \brief MiniPort-LED 例程，通过标准接口实现
 */
void demo_am824_miniport_led_entry (void);

/**
 * \brief MiniPort-ZLG72128 例程，通过标准接口实现
 */
void demo_am824_miniport_zlg72128_entry (void);
