...
/**
 * \brief gpio 硬件层中断（GPIO）例程入口
 *
 * \param[in] key_pin   : 按键引脚号
 * \param[in] led_pin   : led引脚号
 * \param[in] pint_sel  : 触发引脚号
 * \param[in] inum      : 中断号
 */
 ...
 *
 * \param[in] p_hw_i2c  : iic寄存器块
 * \param[in] clkdiv    : 分频值
 * \param[in] addr      : 读取的地址
 * \param[in] sub_addr  : 读取地址长度
 */
...
/**
 * \brief uart  硬件层（dma）例程入口
 *
 * \param[in] p_hw_i2c  : UART存器块
 * \param[in] uclk      : uart时钟
 * \param[in] baudrate  : 波特率
 * \param[in] chan      : 通道号
 */
