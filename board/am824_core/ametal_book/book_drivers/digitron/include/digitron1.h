...
// digitron1_hc595_init_with_softimer()替代digitron1_init_with_softimer()
void digitron1_hc595_init(void);						// 板级初始化
void digitron1_hc595_disp_scan(void);			//动态扫描显示，每5ms扫描一次
void 	digitron1_hc595_init_with_softimer(void);			// 带软件定时器的板级初始化函数

// 设置数码管显示的段码值
// pos的值为0，则设置com0显示的段码；pos的值为1，则设置com1显示的段码
// code为待传送的段码，bit0~bit7与a ~ dp段对应，为0则点亮相应段，为1则熄灭相应段
void digitron1_disp_code_set (uint8_t pos, uint8_t code);

// 设置数码管显示的数字
// pos的值为0，则设置com0显示的数字；pos的值为1，则设置com1显示的数字
// num为待显示的数字，有效范围为0 ~ 9
void digitron1_disp_num_set (uint8_t pos, uint8_t num);

//获取num参数指定数字的段码，返回对应的段码值，num有效范围为0 ~ 9
uint8_t digitron1_num_decode (uint8_t num);

// 设置数码管的闪烁属性
// pos的值为0，则设置com0的闪烁属性；pos的值为1，则设置com1的闪烁属性
// isblink为AM_TRUE，闪烁；isblink为AM_FALSE，正常显示，不闪烁
void digitron1_disp_blink_set (uint8_t pos, am_bool_t isblink);
