...
/**
 * \file
 * \brief zmf159_USB_printer 用户配置文件
...
/**
 * \addtogroup am_zmf159_if_hwconfig_src_usb_printer
 * \copydoc am_zmf159_hwconfig_usb_printer.c
 * @{
 */
...
//    amhw_zmf159_usbd_connect_set(zmf159_USB_DISCONNECT);   /* 断开连接 */
...
am_usbd_printer_handle am_zmf159_usbd_printer_inst_init (void)
{
    return am_usbd_printer_init(&__g_usb_printer_dev,
                                &__g_usbd_printer_info,
                                 am_zmf159_usbd_init(&__g_zmf159_usb_printer_dev, &__g_zmf159_usbd_printer_info));
}
