...
static void digitron_segcode_set (uint8_t code)           // 段码传送函数
{
    int i;
...
static void digitron_com_sel (uint8_t pos)                       // 位码传送函数
{
    int i;
...
void digitron_init1 (void)                                 // 板级初始化函数
{
    ...
    digitron_disp_code_set(0,0xFF);                       // 初始设置缓冲区中的段码值为无效值
    digitron_disp_code_set(1,0xFF);                       // 初始设置缓冲区中的段码值为无效值
}
...
void digitron_disp_scan (void)
{
    ...
    digitron_com_sel(pos);                            // 当前显示位
    if (((g_blink_flag & (1 << pos)) && (cnt <= 49)) || ((g_blink_flag & (1 << pos)) == 0))
          digitron_segcode_set(g_digitron_disp_buf[pos]);     // 传送正常显示段码
...
{
    digitron_disp_scan();                             // 每隔5ms调用扫描函数
}

void digitron_softimer_set (void)
{
...
{
    digitron_init1 ();                                // 板级初始化函数
    digitron_softimer_set();                          // 自动扫描软件定时器
}

void digitron_disp_code_set (uint8_t pos, uint8_t code)   // 传送段码到显示缓冲区函数
{
    g_digitron_disp_buf[pos] = code;
}

void digitron_disp_num_set (uint8_t pos, uint8_t num)     // 传送数字0 ~ 9到显示缓冲区函数
{
    if (num <= 9) 
        g_digitron_disp_buf[pos] = g_segcode_list[num];
}

uint8_t digitron_num_decode (uint8_t num)                 // 获取待显示数字的段码函数
{
    return g_segcode_list[num];
}

void digitron_disp_blink_set (uint8_t pos, am_bool_t isblink)    // 闪烁处理函数
{
    if (isblink) {
        g_blink_flag |= (1 << pos);
    } else {
...
static void digitron_hc595_segcode_set (uint8_t code)				// 传送段码到显示缓冲区函数
{
    hc595_send_data(code);							    // 串行输入并行输出数据
}

void digitron_hc595_init (void)							// 板级初始化函数
{
    int i;
...
void digitron_hc595_disp_scan (void)			// 动态扫描显示函数
{
    static uint8_t pos = 0;								// 初始化com变量
    static uint8_t cnt = 0;  							// 扫描计数器变量

    digitron_hc595_segcode_set(0xFF);         			// 消影，熄灭全部数码管
    digitron_com_sel(pos);                     			// 当前显示位
    if (((g_blink_flag & (1 << pos)) && (cnt <= 49)) ||((g_blink_flag & (1 << pos)) == 0)) {
...
{
    digitron_hc595_disp_scan();                 // 每隔5ms调用扫描函数
}

static void digitron_hc595_softimer_set (void)
{
    am_softimer_init(&g_timer, timer_hc595_callback, NULL);
    am_softimer_start(&g_timer, 5);  			// 启动定时器，每5ms调用一次回调函数
}

void digitron_hc595_init_with_softimer (void)
{
    digitron_hc595_init();
    digitron_hc595_softimer_set();
}
