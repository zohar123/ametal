...
uint8_t digitron_char_decode (char ch)
{
...
void digitron_init2 (void)                            // 板级初始化函数
{
    int i;
    ...
    digitron_disp_code_set2(0,0x00);                 // 初始设置缓冲区中的段码值为无效值
    digitron_disp_code_set2(1,0x00);                 // 初始设置缓冲区中的段码值为无效值
}

void digitron_disp_scan2 (void)
{
    static uint8_t  pos = 0;
    static uint8_t  cnt = 0;                        // 扫描次数记录

    digitron_segcode_set(~0x00);                    // 消影，段码0x00取反
    digitron_com_sel(pos);                          // 当前显示位
    if (((g_blink_flag & (1 << pos))  && (cnt <= 49)) || ((g_blink_flag & (1 << pos)) == 0)) {
        digitron_segcode_set(~g_digitron_disp_buf[pos]);      // 获取缓冲区的段码，段码取反
    }
...
{
    digitron_disp_scan2();                           // 每隔5ms调用扫描显示函数
}

static void digitron_softimer_set (void)
{
    am_softimer_init(&g_timer, timer_callback, NULL);       // 初始化软件定时器
    am_softimer_start(&g_timer, 5);                         // 启动5ms软件定时器
}

void digitron_init_with_softimer2 (void)
{
    digitron_init2 ();                                    // 板级初始化函数
    digitron_softimer_set();                             // 自动扫描软件定时器
}

void digitron_disp_code_set2 (uint8_t pos, uint8_t code)  // 传送段码到显示缓冲器函数
{
...
void digitron_disp_char_set (uint8_t pos, char ch)       // 获取待显示字符的段码函数
{
    digitron_disp_code_set2(pos, digitron_char_decode(ch));
}

void digitron_disp_blink_set2 (uint8_t pos, am_bool_t isblink)
{
    if (isblink) {
        g_blink_flag |= (1 << pos);
    } else {
